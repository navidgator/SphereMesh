...
edges = sparse([]); % Adjacency matrix: each entry defines a pill
...
while size(frontier) ~= 0 % search until all possibile new points eliminated
    ...
    edges(parent_idx,current_idx) = true;
    ...
    for i = 1:size(new_points,1)
        ...
        for j = 1:size(culled_points,1)
            ...
            if delta_theta < 0.1 % remove search points that converged to same angle
                cull = true;
                break;
            end
        end
    ...
    frontier = cat(2,[n+1:n+size(new_points,1);current_idx*ones(1,size(new_points,1))],frontier);
...
[start_inds,end_inds] = find(edges);

start_points = skeleton_points(start_inds,:);
end_points = skeleton_points(end_inds,:);
